import React, { useState } from "react";
import { signInWithGoogle } from "../../firebase";

export default function AuthGate() {
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");

  const handleSignIn = async () => {
    setLoading(true);
    setErr("");
    try {
      await signInWithGoogle();
    } catch (e) {
      setErr(e.message || "Sign-in failed. Ensure Google Auth is enabled in Firebase console.");
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="sp-auth">
      <div className="sp-auth-bg" />
      <div className="sp-auth-card">
        <div className="sp-auth-brand">
          <span className="sp-auth-dot" />
          <span>Skill Development Plan</span>
        </div>
        <h1 className="sp-auth-title">
          Sign in to <em>save your progress</em>
        </h1>
        <p className="sp-auth-sub">
          Track every problem, store your code, save answers, and sync across devices with your friends.
        </p>

        <button className="sp-btn sp-btn-google" onClick={handleSignIn} disabled={loading}>
          <svg width="18" height="18" viewBox="0 0 24 24" style={{ flexShrink: 0 }}>
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          {loading ? "Signing in..." : "Continue with Google"}
        </button>

        {err && <p className="sp-auth-err">{err}</p>}

        <div className="sp-auth-note">
          <strong>Setup note:</strong> Enable "Google" sign-in method and create a Firestore database
          in your Firebase Console if you see errors.
        </div>
      </div>
    </div>
  );
}
